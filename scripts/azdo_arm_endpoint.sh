...
_update_endpoint() {
  ...
  local azdo_endpoint_name="${1}"
  echo -e "Updating the Azure RM service endpoint \"${azdo_endpoint_name}\" in project \"${AZURE_DEVOPS_PROJECT_NAME}\""
  ...
  if [ -z "${azdo_endpoint_id}" ]; then
    ...
    # No option to update the Service Principal using the cli so have to delete it first and recreate in case the Secret was updated
